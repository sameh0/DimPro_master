...
class PrefUtil {
    companion object {
        ...
        fun getTimerState(context: Context): TitleAdapter.TimerState {
            val preferences = PreferenceManager.getDefaultSharedPreferences(context)
            val ordinal = preferences.getInt(TIMER_STATE_ID, 0)
            return TitleAdapter.TimerState.values()[ordinal]
        }

        fun setTimerState(state: TitleAdapter.TimerState, context: Context){
            val editor = PreferenceManager.getDefaultSharedPreferences(context).edit()
